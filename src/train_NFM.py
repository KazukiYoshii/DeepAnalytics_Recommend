...
def make_minibatch(batch_queue, X, y, batch_size):
    ...
    for i in np.random.permutation(range(len(X))):
        ...
        feature2 = np.zeros(feature_dim, dtype=np.int32)
        feature2[keys] = keys
        x2.append(feature2)
...
def one_epoch(model, optimizer, X, y, batch_size, feature_dim, xp, train=True):
    ...
    while(True):
        ...
        loss = model(x1, x2, t)
        loss.backward()
        optimizer.update(model, x1, x2, t)
    ...
    if train:
        print("   train MSE:{}".format(sum_loss / len(X)))
    else:
        print("   test MSE:{}".format(sum_loss / len(X)))

    return model, sum_loss / len(X)
...
def train(model, optimizer, X_train, y_train, X_test, y_test, num_iter,
    ...
    # iterを回す
    last_var_loss = 100000000.0
    for epoch in range(1, num_iter + 1):
        print("epoch {}".format(epoch))
        model, _ = one_epoch(model, optimizer, X_train, y_train, batch_size,
                             feature_dim, xp, train=True)
        if test:
            model, current_var_loss = one_epoch(model, optimizer, X_test, y_test, batch_size,
        ...
        # TDOD: ealry stopping
        if last_var_loss < current_var_loss:
            print("ealry stopping")
            break
        else:
            last_var_loss = current_var_loss
